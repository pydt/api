    ...
    error: (event, cb, error, message) => {
      ...
      if (!message) {
        if (error instanceof module.exports.PydtError) {
          ...
          if (error.extraInfo) {
            custom.extraInfo = error.extraInfo;
          }
        }
      }
      ...
      if (!rollbar) {
        finalCb();
      } else if (error instanceof PydtError && !error.logError) {
        // Log PydtError message as warning even if we ask to suppress it
        rollbar.reportMessageWithPayloadData(error.message, { 
          level: 'warning',
          custom: custom
        }, null, finalCb);
      } else {
        rollbar.handleErrorWithPayloadData(error, { custom: custom }, finalCb);
      }
    }
  },
  rollbarMessage: (message, level, custom) => {
